	...
	$link = mysqli_connect(HOST, USER, PASSWORD, DB_NAME)
        ...
        if(!empty($_POST['email']) && isset($_POST['email']) &&  !empty($_POST['password']) &&  isset($_POST['password']) &&  !empty($_POST['name']) &&  isset($_POST['name']))
        {
          ...
          if(mysqli_num_rows($count) < 1 && mysqli_num_rows($count1) < 1)
          {
            ...
            $mail->Body ='Здравствуйте, ' . $name . '! Спасибо за то, что решили следить за развитием моей вселенной. Осталось только убедиться в том, что Вы человек. Пожалуйста, подтвердите адрес вашей электронной почты, перейдя по ссылке ниже, и Вы сможете в полном объёме начать использовать ваш аккаунт на сайте. <br/> <br/> 
